...
usage () {
  echo "Usage:"
  echo "install-trader-operator.sh CLUSTER_NAME API_KEY"
}
...
echo "Installing Trader Lite Operator in namespace openshift-operators ..."
echo ""

echo "Creating subscription to TraderLite operator in project openshift-operators..."
echo "Check if TraderLite subscription exists in project openshift-operators ..."
oc get Subscription traderlite-operator -n openshift-operators  >/dev/null 2>&1

if [ $? -ne 0 ]; then

    operator-sdk run bundle $TRADERLITE_OPERATOR_BUNDLE --install-mode AllNamespaces --namespace openshift-operators
    if [ $? -ne 0 ]; then
        echo "Error creating subscription to TraderLite operator in project $PROJECT"
        exit 1
    fi

else
   echo "Subscription to traderlite-operator already exists in project openshift-operators, skipping create ..."
   echo "Check if  Operator is successfully Deployed - give up after 5 minutes"
   printf "Status: Querying CSV in project openshift-operators.."
   retry 60 checkCSVComplete $TRADERLITE_CSV openshift-operators
   if [ $? -ne 0 ]; then
      echo "Error: timed out waiting for operators in project openshift-operators"
      exit 1
   fi
fi
...
# operator-sdk cleanup traderlite-operator -n openshift-operators --delete-all
# oc delete cm traderlite-install-progress -n default